...
CI.dist=sapply(c(1:length(timelist)), function(x) {
  etas.CI(timelist[x],times[1:n.events[x]],mags[1:n.events[x]],
          m0=3,mu=.1687,K=.04225,alpha=1.034/log(10),c=.01922,p=1.222)
})
# Find values at event times
CI.list=sapply(c(1:n.training), function(x) {
  etas.CI(times[1+x],times[1:x],mags[1:x],
          m0=3,mu=.1687,K=.04225,alpha=1.034/log(10),c=.01922,p=1.222)
})
...
CI2.dist=sapply(c(1:length(timelist)), function(x) {
  etas.CI(timelist[x],times[1:n.events[x]],mags[1:n.events[x]],
          m0=3,mu=mu.hat,K=K.hat,alpha=alpha.hat/log(10),c=c.hat,p=p.hat)
})

CI2.list=sapply(c(1:n.training), function(x) {
  etas.CI(times[1+x],times[1:x],mags[1:x],
          m0=3,mu=mu.hat,K=K.hat,alpha=alpha.hat/log(10),c=c.hat,p=p.hat)
})
...
# Improve performance by avoiding calculation of 5.8^mags in each loop.
power_mags=5.8^mags
